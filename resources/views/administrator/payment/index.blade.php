                        ...
                        <tbody>
                            @foreach ($payments as $index => $payment)
                                <tr>
                                    ...
                                    <td>{{ $payment->email }}</td>
                                    <td><img src="{{ asset('bukti_payment/' . $payment->gambar) }}" alt="image" class="img-fluid"></td>
                                    <td>{{ $payment->total }}</td>
                                    <td>{{ $payment->status }}</td>
                                    <td>{{ $payment->payment_method }}</td>
                                    <td>
                                        <div class="d-flex justify-content-center">
                                            <a href="{{ route('administrator.payment.edit', $payment->id) }}"
                                                class="btn btn-success btn-sm d-inline-flex align-items-center justify-content-center me-2"
                                                style="width: 32px; height: 32px;">
                                                <i class="fa fa-check"></i>
                                            </a>
                                            <button
                                                data-url="{{ route('administrator.payment.destroy', $payment->id) }}"
                                                type="button"
