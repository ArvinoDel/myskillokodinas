...
@section('container')
    @if (Auth::check())
                        ...
                        @else
                            @if ($vidActive)
                                <div class="grid grid-rows-1 md:grid-cols-2">
                                    <div class="grid grid-flow-row text-center mx-4 md:mx-6 p-4 max-lg:mx-16 max-lg:p-4">
                                        <h3 class="font-bold text-lg md:text-xl py-2 max-lg:text-lg">Selamat Datang,
                                            Pengguna
                                            Setia! Akses Materi Kapan Saja!</h3>
                                        <h3 class="text-sm md:text-md py-2 max-lg:text-base">Sebagai pengguna berlangganan,
                                            nikmati seluruh materi premium kami,
                                            termasuk video edukasi, modul interaktif, dan webinar dari para pakar industri.
                                    ...
                                    <div class="grid grid-rows-1 md:mx-0 lg:mx-12">
                                        <div id="divMateri" class="mx-4 md:mx-8 max-lg:mx-12">
                                            <h3 class="text-gray-500 font-semibold py-4 mx-0 md:mx-7 md:mt-1.5 lg:mx-6">
                                                Materi</h3>
                                            @foreach ($materi->isimateri as $isi)
                                                <button
                                                    onclick="openFile('{{ asset('../files/' . $isi->file) }}', '{{ $isi->file }}')"
                                                ...
                                                <button class="w-full"
                                                    ...
                                                    <div class="py-2 flex items-center justify-between max-lg:py-3">
                                                        <div
                                                            ...
                                                            <h3
                                                                class="text-sm md:text-base max-lg:text-base cursor-pointer">
                                                                {{ $task->judul_tugas }}
                                                            </h3>
                                                        </div>
                                                        <div class="ml-auto">
                                                            <i class="fa-solid fa-upload text-lg md:text-xl max-lg:text-lg cursor-pointer"
                                                                onclick="event.stopPropagation(); toggleModal(true, '{{ $task->id_tugas }}')"
                                                                ></i>
                                                            <!-- Tambahkan event.stopPropagation() -->
                                                        </div>
                                                    </div>
                                                </button>
                                            @endforeach
                                        ...
                                        <div id="photoUploadModal"
                                            ...
                                            <div
                                                ...
                                                <form action="{{ url('e-learning/materi') }}"
                                                    method="POST" enctype="multipart/form-data"
                                                    class="px-4 py-2 form-ajax">
                                                    @csrf
                                                    ...
                                                    <div id="file-preview"
                                                        ...
                                                        <input id="fileInput" type="file"
                                                            class="hidden" accept="image/*,application/pdf,video/*"
                                                    ...
                                                    <div class='mb-4'>
                                                        <input name="file" type="file" accept="image/*,application/pdf,video/*" />
                                                    </div>
                                                    <div class="mb-4">
            ...
            <section class="md:p-8">
                <div class="container mx-auto px-4">
                    ...
                    <div class="text-start py-4">
                        ...
                        <h3 class="text-sm sm:text-base md:text-lg font-semibold py-1">Veronica Gabriella - Senior
                            Copywriter
                            SATU Dental Group</h3>
                        <p class="text-xs sm:text-sm md:text-md py-1">Copywriting menjadi bagian dalam digital marketing
                            sebagai
            ...
            $(function() {
                ...
                $('.form-ajax').each(function() {
                    $(this).bind('submit', function(e) {
                        ...
                        let form = $(this); // Mengambil elemen form

                        $.ajax({
                            url: form.prop('action'), // Mengambil action dari form
                            data: new FormData(this), // Mengambil data form termasuk file
                            cache: false,
                            async: true,
                            type: 'post',
                            method: 'post',
                            dataType: 'json',
                            contentType: false, // Agar tidak mengatur tipe konten secara otomatis
                            processData: false, // Agar data tidak diubah secara otomatis
                            success: function(data) {
                                ...
                                if (data.success === false) {
                                    ...
                                    Swal.fire({
                                        ...
                                        // Setelah pesan sukses muncul, redirect ke halaman yang diberikan
                                        window.location.href = data.url; // Menggunakan window.location.href untuk redirect
                                    });
                                }
                            }
                        });
        ...
        <script>
            ...
            function toggleModal(show, tugas) {
            ...
            function openTaskModal(judul, deskripsi, file) {
                document.getElementById('taskTitle').textContent = judul;
                document.getElementById('taskDescription').textContent = deskripsi;
                document.getElementById('id_tugas').value = file; // Set the hidden input value
