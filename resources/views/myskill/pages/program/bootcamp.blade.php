...
<section class="bootcamp-program w-screen">
        ...
        <div class="flex overflow-x-auto space-x-4 mx-5 px-1 pb-4 no-scrollbar">
            ...
            <div class="bg-white p-4 rounded-2xl shadow-md max-w-[220px] sm:max-w-[180px] md:max-w-[200px] flex-shrink-0">
                ...
                <a href="{{ $testimoni->link }}">
                    <button class="w-full bg-violet-500 text-white py-2 rounded-md font-semibold">Baca Cerita</button>
                </a>
            </div>
            @endforeach
        </div>
        ...
        <a href="/bootcamp/digital-marketing/{{ $bootcamp->id_bootcamp }}">
            <div class="bg-white mt-2 mb-2 border rounded-lg shadow-md max-sm:p-2 max-sm:mr-2 max-sm:flex max-sm:flex-col max-sm:items-center max-sm:justify-start">
                ...
                <div class="text-left sm:text-left sm:mt-4">
                    ...
                    <div class="flex items-center justify-start max-sm:mt-1 text-gray-500">
                        <i class="fas ml-2 fa-calendar-alt mr-2"></i>
                        <p class="text-sm">{{ $bootcamp->batch->last()->tanggal_mulai ?? 'Tidak ada batch yang tersedia' }}</p>
                    </div>
                    <div class="flex items-center justify-start max-sm:mt-1 text-gray-500">
                        <i class="fas ml-2 fa-tag mr-2"></i>
                        <p class="text-sm">
                            Rp {{ number_format($bootcamp->harga_diskon, 0, ',', '.') }}
                            <span class="line-through text-xs/tight max-sm:hidden text-red-500">
    ...
    <section class="w-full h-auto rounded-b-3xl bg-orange-100 lg:flex items-center mt-12 p-4">
        ...
        <div class="mx-auto">
            ...
            <br>
            <p class="w-4/5 ml-4">Miliki akses ratusan konten elearning Pandai Digital serta dukungan corporate training untuk perusahaan. Miliki juga berbagai fitur khusus untuk mendorong employee performance and development.</p>
            <br>
            <a href="/corporate-service" type="button" class="ml-4 focus:outline-none text-black bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-4 py-3 me-2 mb-2 dark:focus:ring-yellow-900">Hubungi Tim Pandai Digital</a>
        </div>
    </section>
</section>
@endsection